...
func getUserPreferredChannel(c *gin.Context, modelName string, usingGroup string) (*model.Channel, bool) {
	if usingGroup == "auto" {
		return nil, false
	}
	...
	channelID, ok := setting.PreferredChannels[usingGroup]
	if !ok || channelID <= 0 {
		return nil, false
	}
	...
	if !channelSupportsRequestPath(channel, c.Request.URL.Path, modelName) || !model.IsChannelEnabledForGroupModel(usingGroup, modelName, channelID) {
		return nil, false
	}
	return channel, true
}
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	common.SetContextKey(c, constant.ContextKeyChannelRetryTimes, channel.RetryTimes)
